...
public abstract class ABizLogic implements IHttpUtility {
	...
	public <T> T doGet(final Setting actionSetting, final Params params, Class<T> responseCls) throws TaskException {
		...
		// 配置的缓存模式
		String cacheUtilityStr = SettingUtil.getSettingValue(actionSetting, Constant.CACHE_UTILITY);
		...
		// 内存缓存
		if (actionSetting.getExtras().containsKey(Constant.MEMORY_CACHE_UTILITY)) {
			String memoryCacheUtilityStr = actionSetting.getExtras().get(Constant.MEMORY_CACHE_UTILITY).getValue();
		...
		// 缓存是否在action中配置打开
		boolean cacheEnable = actionSetting.getExtras().containsKey(Constant.CACHE_ENABLE) ? Boolean.parseBoolean(actionSetting.getExtras()
				.get(Constant.CACHE_ENABLE).getValue()) : false;

		if (cacheEnable && (mCacheMode == CacheMode.cachePriority || mCacheMode != CacheMode.disable)) {
			// 拉取内存
			// SettingUtility.getIntSetting(Constants.MEMORY_CACHE_VALIDITY) > 0
...
//		return mGson.toJson(o);
	...
	private HttpConfig cloneHttpConfig(HttpConfig config, Setting actionSetting) {
		try {
			...
			if (actionSetting != null && actionSetting.getExtras().containsKey(Constant.BASE_URL))
				mConfig.baseUrl = actionSetting.getExtras().get(Constant.BASE_URL).getValue().toString();
